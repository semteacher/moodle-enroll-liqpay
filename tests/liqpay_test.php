...
class enrol_liqpay_testcase extends advanced_testcase {
    ...
    protected function disable_plugin() {
        $enabled = enrol_get_plugins(true);
        unset($enabled['liqpay']);
        $enabled = array_keys($enabled);
        set_config('enrol_plugins_enabled', implode(',', $enabled));
    }

    public function test_basics() {
        $this->assertFalse(enrol_is_enabled('liqpay'));
        $plugin = enrol_get_plugin('liqpay');
        $this->assertInstanceOf('enrol_liqpay_plugin', $plugin);
        $this->assertEquals(ENROL_EXT_REMOVED_SUSPENDNOROLES, get_config('enrol_liqpay', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $liqpayplugin->sync(new null_progress_trace());
    }
    ...
    public function test_expired() {
        ...
        /** @var enrol_liqpay_plugin $liqpayplugin  */
        $liqpayplugin = enrol_get_plugin('liqpay');
        ...
        $liqpayplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $liqpayplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $liqpayplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $liqpayplugin->enrol_user($instance2, $user1->id, $studentrole->id, 0, 0);
        $liqpayplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now-60*60);
        $liqpayplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now+60*60);

        $liqpayplugin->enrol_user($instance3, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $liqpayplugin->enrol_user($instance3, $user4->id, $teacherrole->id);
        ...
        $liqpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_KEEP);
        $code = $liqpayplugin->sync($trace);
        $this->assertSame(0, $code);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        $this->assertEquals(9, $DB->count_records('role_assignments'));


        $liqpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $liqpayplugin->sync($trace);
        $this->assertEquals(9, $DB->count_records('user_enrolments'));
        ...
        $liqpayplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the liqpay enrol plugin.
        ...
        $actions = $plugin->get_user_enrolment_actions($manager, $ue);
        // Teachers don't have the enrol/liqpay:unenrol capability by default, but have enrol/liqpay:manage.
