...
use enrol_liqpay\privacy\provider;
use core_privacy\local\request\writer;

/**
 * Class enrol_liqpay_privacy_provider_testcase.
 *
 * @copyright  2018 Shamim Rezaie <shamim@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_liqpay_privacy_provider_testcase extends \core_privacy\tests\provider_testcase {
    ...
    protected function setUp() {
        ...
        $liqpayplugin->enrol_user($enrolinstance1, $this->student1->id, $studentrole->id);
        $this->create_enrol_liqpay_record(
            $this->businessuser1,
            $this->receiveruser1,
            $this->course1,
            $this->student1,
        ...
        $liqpayplugin->enrol_user($enrolinstance2, $this->student2->id, $studentrole->id);
        ...
        // First in course1.
        $liqpayplugin->enrol_user($enrolinstance1, $this->student12->id, $studentrole->id);
        $this->create_enrol_liqpay_record(
            $this->businessuser2,
            $this->receiveruser1,
            $this->course1,
        ...
        // Then in course2.
        $liqpayplugin->enrol_user($enrolinstance2, $this->student12->id, $studentrole->id);
        $this->create_enrol_liqpay_record(
            $this->businessuser2,
            $this->receiveruser2,
            $this->course2,
        ...
        $liqpayplugin->enrol_user($enrolinstance1, $this->student3->id, $studentrole->id);
        $this->create_enrol_liqpay_record(
            $this->businessuser3,
            $this->receiveruser3,
            $this->course3,
    ...
    public function test_get_metadata() {
        $collection = new collection('enrol_liqpay');
        $newcollection = provider::get_metadata($collection);
        $itemcollection = $newcollection->get_collection();
        $this->assertCount(2, $itemcollection);

        $location = reset($itemcollection);
        $this->assertEquals('liqpay.com', $location->get_name());
        $this->assertEquals('privacy:metadata:enrol_liqpay:liqpay_com', $location->get_summary());
        ...
        $this->assertEquals('enrol_liqpay', $table->get_name());
        ...
        $this->assertArrayHasKey('currency_debit', $privacyfields);
        $this->assertArrayHasKey('payment_status', $privacyfields);
        $this->assertArrayHasKey('end_date', $privacyfields);
        $this->assertArrayHasKey('create_date', $privacyfields);
        $this->assertArrayHasKey('acq_id', $privacyfields);
        $this->assertArrayHasKey('payment_id', $privacyfields);
        $this->assertArrayHasKey('liqpay_order_id', $privacyfields);
    ...
    public function test_export_user_data() {
        ...
        $this->export_context_data_for_user($this->student1->id, $coursecontext1, 'enrol_liqpay');
        $writer = writer::with_context($coursecontext1);
        $this->assertTrue($writer->has_any_data());

        $data = $writer->get_data([get_string('transactions', 'enrol_liqpay')]);
    ...
    public function test_export_user_data_not_enrolled() {
        ...
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student2->id, $coursecontext1, 'enrol_liqpay');
        $writer = writer::with_context($coursecontext1);
    ...
    public function test_export_user_data_no_enrolment() {
        ...
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student0->id, $coursecontext1, 'enrol_liqpay');
        $writer = writer::with_context($coursecontext1);
        $this->assertFalse($writer->has_any_data());
    }

    public function test_export_user_data_multiple_liqpay_history() {
        $coursecontext2 = context_course::instance($this->course2->id);

        $this->setUser($this->student2);
        // Export all of the data for the context.
        $this->export_context_data_for_user($this->student2->id, $coursecontext2, 'enrol_liqpay');
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertEquals(
                2,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course1->id])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course2->id])
        );
    ...
    public function test_delete_data_for_user_from_single_context() {
        ...
        $this->assertEquals(
                1,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course1->id, 'userid' => $this->student12->id])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course2->id])
        ...
        $this->assertEquals(
                1,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course2->id, 'userid' => $this->student12->id])
        );

        // Delete data for user.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->student12, 'enrol_liqpay',
                [$coursecontext1->id]);
    ...
    public function test_delete_data_for_user_for_business_user() {
        ...
        $this->assertEquals(
                3,
                $DB->count_records('enrol_liqpay', ['business' => $this->businessuser1->email])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_liqpay', ['business' => $this->businessuser2->email])
        );

        // Delete data for user in $coursecontext1.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->businessuser1, 'enrol_liqpay',
                [$coursecontext1->id]);
        provider::delete_data_for_user($contextlist);
        ...
        $this->assertEquals(
                0,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course1->id, 'business' => $this->businessuser1->email])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_liqpay', ['business' => $this->businessuser1->email])
        ...
        $this->assertEquals(
                1,
                $DB->count_records('enrol_liqpay', ['courseid' => $this->course1->id, 'business' => ''])
        );
        $this->assertEquals(
                2,
                $DB->count_records('enrol_liqpay', ['business' => $this->businessuser2->email])
    ...
    public function test_delete_data_for_user_for_receiver_user() {
        ...
        $this->assertEquals(
                2,
                $DB->count_records('enrol_liqpay', ['receiver_email' => $this->receiveruser1->email])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_liqpay', ['receiver_email' => $this->receiveruser2->email])
        );

        // Delete data for user.
        $contextlist = new \core_privacy\local\request\approved_contextlist($this->receiveruser1, 'enrol_liqpay',
                [$coursecontext1->id]);
        provider::delete_data_for_user($contextlist);
        ...
        $this->assertEquals(
                2,
                $DB->count_records('enrol_liqpay', ['receiver_email' => ''])
        );
        $this->assertEquals(
                3,
                $DB->count_records('enrol_liqpay', ['receiver_email' => $this->receiveruser2->email])
    ...
    /**
     * Helper function to create an enrol_liqpay record.
     *
    ...
    protected function create_enrol_liqpay_record($business, $receiver, $course, $user, $enrol, $payment_id, $time) {
        ...
        $DB->insert_record('enrol_liqpay', $liqpaydata);
    }
    ...
    public function test_get_users_in_context() {
        ...
        $userlist1 = new \core_privacy\local\request\userlist($coursecontext1, 'enrol_liqpay');
        ...
        $userlist2 = new \core_privacy\local\request\userlist($coursecontext2, 'enrol_liqpay');
        ...
        $userlist3 = new \core_privacy\local\request\userlist($coursecontext3, 'enrol_liqpay');
    ...
    public function test_delete_data_for_users() {
        ...
        $this->assertEquals(
                ...
                $DB->get_fieldset_select('enrol_liqpay', 'courseid', 'userid = ?', [$this->student12->id]),
                '', 0.0, 10, true
        );
        $this->assertEquals(
                [$this->course1->id, $this->course2->id, $this->course2->id],
                $DB->get_fieldset_select('enrol_liqpay', 'courseid', 'business = ?',
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($coursecontext1, 'enrol_liqpay',
    ...
    public function test_delete_data_for_users_business() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol_liqpay', ['business' => $this->businessuser1->email]));
        $transactions = $DB->get_records('enrol_liqpay', [
        ...
        $this->assertEquals(2, $DB->count_records('enrol_liqpay', ['business' => $this->businessuser1->email]));
        $this->assertEquals(0, $DB->count_records('enrol_liqpay', [
            'courseid' => $this->course1->id,
            'business' => $this->businessuser1->email
        ]));
        ...
        $newtransaction = $DB->get_record('enrol_liqpay', ['id' => $transaction->id]);
    ...
    public function test_delete_data_for_users_receiver() {
        ...
        $this->assertEquals(2, $DB->count_records('enrol_liqpay', ['receiver_email' => $this->receiveruser1->email]));
        $transactions = $DB->get_records('enrol_liqpay', [
            'courseid' => $this->course1->id,
        ...
        $this->assertEquals(0, $DB->count_records('enrol_liqpay', ['receiver_email' => $this->receiveruser1->email]));
