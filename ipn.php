...
set_exception_handler(\enrol_liqpay\util::get_exception_handler());
...
$liqpayaddr = empty($CFG->useliqpaysandbox) ? 'ipnpb.liqpay.com' : 'ipnpb.sandbox.liqpay.com';
$c = new curl();
$options = array(
    'returntransfer' => true,
    'httpheader' => array('application/x-www-form-urlencoded', "Host: $liqpayaddr"),
    'timeout' => 30,
    'CURLOPT_HTTP_VERSION' => CURL_HTTP_VERSION_1_1,
);
$location = "https://$liqpayaddr/cgi-bin/webscr";
$result = $c->post($location, $req, $options);

if ($c->get_errno()) {
    throw new moodle_exception('errliqpayconnect', 'enrol_liqpay', '', array('url' => $liqpayaddr, 'result' => $result),
        json_encode($data));
}
...
if (strlen($result) > 0) {
    if (strcmp($result, "VERIFIED") == 0) {          // VALID PAYMENT!
        ...
        if ($data->payment_status != "Completed" and $data->payment_status != "Pending") {
            ...
            \enrol_liqpay\util::message_liqpay_error_to_admin("Status not completed or pending. User unenrolled from course",
                                                              $data);
            die;
        }
        ...
        if ($data->payment_status == "Pending" and $data->pending_reason != "echeck") {
            ...
            $eventdata->subject           = "Moodle: LiqPay payment";
            $eventdata->fullmessage       = "Your LiqPay payment is pending.";
            $eventdata->fullmessageformat = FORMAT_PLAIN;
            $eventdata->fullmessagehtml   = '';
            $eventdata->smallmessage      = '';
            message_send($eventdata);

            \enrol_liqpay\util::message_liqpay_error_to_admin("Payment pending", $data);
            die;
        }

        // If our status is not completed or not pending on an echeck clearance then ignore and die
        // This check is redundant at present but may be useful if liqpay extend the return codes in the future
        ...
        if (core_text::strtolower($recipient) !== core_text::strtolower($plugin->get_config('liqpaybusiness'))) {
            \enrol_liqpay\util::message_liqpay_error_to_admin("Business email is {$recipient} (not ".
                    $plugin->get_config('liqpaybusiness').")", $data);
            die;
        }
        ...
        $DB->insert_record("enrol_liqpay", $data, false);
        throw new moodle_exception('erripninvalid', 'enrol_liqpay', '', null, json_encode($data));
    }
}
